...
class _MealDetailPageState extends State<MealDetailPage> {
  ...
  void _updateResearch(String string) {
    setState(() {
      if (string != null && string != "") {

        _ricette = null;
        _ricette = DatabaseService.instance.searchRicetteByName(string);
      }
    });
  }
...
void _insertRicettaAndPop(Ricette _ricetta){
 DatabaseService.instance.updateMealRicette(_ricetta,mealInput);
  ...
  Widget _ricetteList() {
    return ListView.separated(
        ...
        itemBuilder: (context, index) {
          return Padding(
            padding: const EdgeInsets.all(8.0),
            child: GestureDetector(child: SingleRicetta(_ricette[index]),onTap:()=>_insertRicettaAndPop(_ricette[index])),
          );
        });
  }
}

...
class SingleProductWidget extends StatelessWidget {
  ...
  Future<void> _askSpesaDelete(BuildContext context) async {
    await showCupertinoDialog(
        context: context,
        builder: (ctx2) {
          return CupertinoAlertDialog(
            title: Text("Vuoi eliminare questa spesa con 0 prodotti?"),
            actions: [
    ...
    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [
            ColorCostant.colorMap[_product.color].withOpacity(0.3),
            Colors.white,
          ],
        ),
      ),
  ...
  Widget _productCard() {
    return ListTile(
      ...
      dense: true,
      leading: (_product.image != null && _product.image.isNotEmpty)
          ? Image.network(
              _product.image,
              height: 40,
            )
